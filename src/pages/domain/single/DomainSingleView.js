    ...
    return (
      <Segment basic loading={!ready}>
        ...
        <div>
                  ...
                  {Object.keys(uiSchema[grouping]).map(property =>
                    <Grid.Row key={property} style={{paddingTop: '0'}}>
                      ...
                      <Grid.Column width={11}>
                        {convertDataToView(data[property], lds.producer, uiSchema[grouping][property])}
                      </Grid.Column>
                    </Grid.Row>
                  )}
                </Grid>
          ...
          <Divider hidden />
          <Button floated='right' color='orange' icon='edit outline' as={Link} to={`/${domain.route}/${data.id}/edit`}
                  content={UI.EDIT[languageCode]} />
        </div>
        }
